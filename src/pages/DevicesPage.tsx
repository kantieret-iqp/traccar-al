...
import { Plus, Pencil, Trash2, Copy, Check, Clock, User } from 'lucide-react'
...
function DeviceModal({ device, onSave, onClose }: {
  ...
  // When category changes, reset icon to first of that category
  ...
  const catLabel: Record<string, string> = { vehicle: '🚗 Automjet', person: '👤 Person', animal: '🐕 Kafshë', asset: '📦 Pasuri' }
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#161B22] border border-white/[0.08] rounded-2xl p-6 w-full max-w-md fade-in max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={submit} className="flex flex-col gap-4">
                ...
                <button key={cat} type="button" onClick={() => handleCategory(cat)}
                  className={`py-2 px-1 rounded-lg text-xs font-semibold transition-all text-center ${
                    form.category === cat
                      ...
                      : 'bg-[#0D1117] border border-white/[0.08] text-[#7D8590] hover:border-white/20'
                  }`}>
                  {catLabel[cat]}
                </button>
              ))}
          ...
          <div>
            <label className="block text-xs font-mono text-[#7D8590] mb-1.5">
              {form.category === 'person' ? 'Personi (llogaria e tij)' :
               form.category === 'animal' ? 'Pronari i kafshës' : 'Shofer / Pronari'}
            </label>
            ...
            <p className="text-[10px] font-mono text-[#7D8590] mt-1">
              Nëse caktohet pronar, ai user sheh vetëm këtë pajisje në dashboard-in e tij.
            </p>
          </div>
              ...
              {icons.map(ic => (
                <button key={ic} type="button" onClick={() => setForm(p => ({ ...p, icon: ic }))}
                  className={`w-9 h-9 rounded-lg text-lg flex items-center justify-center transition-all ${
                    form.icon === ic
                      ? 'bg-[rgba(0,255,135,0.15)] border border-[rgba(0,255,135,0.3)]'
...
export default function DevicesPage() {
  ...
  const categories = ['all', 'vehicle', 'person', 'animal', 'asset']
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        {[
          ...
          <button key={c.key} onClick={() => setFilterCat(filterCat === c.key ? 'all' : c.key)}
            className={`bg-[#161B22] border rounded-xl p-3 text-left transition-all ${
              filterCat === c.key ? 'border-[rgba(0,255,135,0.3)] bg-[rgba(0,255,135,0.05)]' : 'border-white/[0.05] hover:border-white/10'
            }`}>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.05]">
                {['Pajisja','Kategoria','Pronari','Statusi','Token ID','Parë herën e fundit',''].map(h => (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => copyToken(dev.identifier)}
                        className="p-1.5 rounded-lg hover:bg-white/[0.05] text-[#7D8590] hover:text-[#00FF87] transition-all flex-shrink-0">
